  ...
  function vibration(){						
	...
	$(options.node).parent('p').find('.right2,.err2').remove();  
        if(status==1){//radio或checkbox控件 验证成功
			...
			$(options.node).parent('p').append("<span class='err2'>"+options.errmsg+"</span");
				resultbool=false;  
            }else{//验证成功
            ...
            if(!isOk){//验证失败
					...
					if(status==2){
						$(options.node).after("<span class='err2'><img src='./images/register-error2.png'>"+options.errmsg[1]+"</span>");
					}else if(status==3){
						$(options.node).after("<span class='err2'><img src='./images/register-error2.png'>"+options.errmsg[0]+"</span>");
					}else{
						$(options.node).after("<span class='err2'><img src='./images/register-error2.png'>"+options.errmsg+"</span>");
							}
					resultbool=false;
			}else{//验证成功

					 $(options.node).after("<span class='right2'><img src='./images/register-good.png'> </span>");
			 }
        } 
    }
    $.extend({
