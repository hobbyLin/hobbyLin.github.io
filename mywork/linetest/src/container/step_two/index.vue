...
<!--         <div class="gray-btn" @click="toSuccess" style="margin-top: .22rem;margin-bottom: .22rem;" id="stepTwoBtn">确认上传</div> -->
        <div class="step-two-btn" @click="toSuccess">
            <div class="step-two-gray-btn" id="stepTwoBtn">确认上传</div>
        </div>
    </div>
</template>
        ...
        methods: {
            ...
            changeBtnColor(){
                if(this.faceImgInformation.result !== undefined && this.backImginformation.result !== undefined){
                    let stepTwoBtn = DomTools.$('#stepTwoBtn');
                    DomTools.setClass(stepTwoBtn, 'step-two-orange-btn');
                    //this.$('#stepTwoBtn').setAttribute('class', 'step-two-orange-btn');
                }
            },
            readFile (dict) {
                this.direction = dict;
                // let chooseArray = this.$('#choose');
                let chooseArray = DomTools.$(`#choose_${this.direction}`);
                ...
                // let files = Array.prototype.slice.call(chooseArray.files);
                ...
                reader.onload = () => {
                    ...
                    if (fileSize <= 200 * 1024) {
                        img = null;
                        DomTools.$(`#img_${this.direction}`).innerHTML = `<img src="${result}"/>`;
                        let addOkImg = DomTools.$(`.step-two-${this.direction}`);
                        DomTools.setClass(addOkImg, `pic-box step-two-${this.direction}`);
                        DomTools.$(`#${this.direction}-text`).innerText = ' ';
                        if(dict === 'face'){
            ...
            compress (img) {
                ...
                DomTools.$(`#img_${this.direction}`).innerHTML = `<img src="${ndata}"/>`;
