...
<script>
	...
	export default {
	    ...
	    methods: {
	    	toVideo () {
	    		services.videoAuthentication().then((data) => {
	    			//todo 业务处理
	    			if(data.responseCode === '000000' && data.data.checktime){
	    				ald.nav.forward({
	    				...
	    				dialog.open({
			                ...
			                title: '温馨提示',
			                text: `<div>
			                        <div style="font-size: 0.3rem; color: #666">远程视频身份认证时间是<span style="color: #f37937;font-size: 0.30rem;">${this.setting}</span>,此时间范围内才可以进行远程视频身份认证。</div>
			                    </div>`,
			                showOneBtn: true,
			                oneBtnText: '我知道了',
